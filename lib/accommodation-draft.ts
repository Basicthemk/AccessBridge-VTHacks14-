...
const INSTRUCTION = (profile: DisabilityProfile, lecture?: DraftLecture) => `You write a short, polite email from a college student to a professor asking for learning accommodations.
${ASKS[profile]}
Rules:
...
- End with "Thank you," on one line and then ${NAME_PLACEHOLDER} on the next line. Write ${NAME_PLACEHOLDER} exactly like that.${lectureBlock(lecture)}`;
...
export async function generateAccommodationDraft(profile: DisabilityProfile, lecture?: DraftLecture): Promise<string> {
  ...
  try {
    return await generateWithFallback(ai, {
      ...
      config: { systemInstruction: INSTRUCTION(profile, lecture), temperature: 0.5, maxOutputTokens: 8192 },
      parse: (response) => {
